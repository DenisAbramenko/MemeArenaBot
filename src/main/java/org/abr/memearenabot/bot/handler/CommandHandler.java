...
public class CommandHandler {
    ...
    private void handleAdminCommand(Long chatId, UserSession session, User user) {
        ...
        if (userService.isAdmin(user.getTelegramId())) {
            ...
            try {
                SendMessage adminMessage = createMessage(
                        chatId, 
                        messageService.getAdminWelcomeMessage(),
            ...
            try {
                SendMessage accessDeniedMessage = createMessage(
                        chatId, 
                        messageService.getMessage("admin.access.denied"));
    ...
    private void handleStartCommand(Long chatId, UserSession session, User user) {
        ...
        try {
            ...
            bot.execute(message);
            logger.debug("{}Sent welcome message to chat ID: {}", LOG_PREFIX, chatId);
        } catch (TelegramApiException e) {
    ...
    private void handleTemplateCommand(Long chatId, UserSession session) {
        ...
        try {
            ...
            SendMessage message = createMessage(
                    chatId,
                    messageService.getTemplateChooseMessage(),
