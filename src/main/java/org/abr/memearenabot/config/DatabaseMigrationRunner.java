    ...
    public CommandLineRunner migrateDatabaseSchema(JdbcTemplate jdbcTemplate) {
        return args -> {
            ...
            try {
                ...
                Integer constraintCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM pg_constraint WHERE " +
                        "conname = 'meme_type_check'", Integer.class);
                if (constraintCount != null && constraintCount == 0) {
                    jdbcTemplate.execute("ALTER TABLE memes ADD CONSTRAINT meme_type_check CHECK (type IN " +
                ...
                jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_contest_entries_contest_id ON contest_entries" +
                        "(contest_id)");
                jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_contest_entries_meme_id ON contest_entries" +
                        "(meme_id)");
                ...
                jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS contests (" + "id BIGSERIAL PRIMARY KEY," + "title " +
                        ...
                        " TIMESTAMP NOT NULL," + "status VARCHAR(20) NOT NULL," + "prize_description VARCHAR(1000)," + "max_entries_per_user INTEGER NOT NULL DEFAULT 3" + ")");
                ...
                jdbcTemplate.execute("CREATE TABLE IF NOT EXISTS contest_votes (" + "id BIGSERIAL PRIMARY KEY," +
                        "entry_id BIGINT NOT NULL," + "user_id BIGINT NOT NULL," + "voted_at TIMESTAMP NOT NULL " +
                        "DEFAULT NOW()," + "CONSTRAINT uk_user_entry_vote UNIQUE (user_id, entry_id)" + ")");
